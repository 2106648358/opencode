...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user mentions:**
   ...
   - "show workflows" or "what workflows" → run `opencode openspec schema list --json` and let them choose
...
5. **Get instructions for the first artifact**
   ...
   ```bash
   opencode openspec instructions <first-artifact-id> --change "<name>"
   ```
